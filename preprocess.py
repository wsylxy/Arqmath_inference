...
from pya0 import tokenize as tex_tokenize
...
# def preprocess_for_transformer1(text, math_vocab=None, num_tokenizer_ver=1):
#     output = ''
...
def preprocess_for_transformer_old(text, math_vocab=None, num_tokenizer_ver=2):
    output = ''
...
def count_textual_content_pya0(eq): #consecutive alphabet are counted as text
    text_len = 0
    for i in range(len(eq)):
        if i >= 1 and len(eq[i-1].strip('$')) == 1 and eq[i-1].strip('$').isalpha():
            if len(eq[i].strip('$')) == 1 and eq[i].strip('$').isalpha():
                if i == len(eq)-1:  #if the current alpha is the last token
                    text_len += 2
                elif i < len(eq)-1:
                    text_len += 1
    return text_len
...
if __name__ == '__main__':
    ...
    math = r'S_◹ = \sum_{1 \leq j \leq k \leq n} a_ja_k = \sum_{1 \leq k \leq j \leq n} a_ka_j = \sum_{1 \leq k \leq j \leq n} a_ja_k = S_◺'
    math = r'S^{−1}AS$x1+$S'
    math = r'$$123∫_0^1F\left(t,0\right)\left(1,0\right)dt+∫_0^{\frac π 23} F\left(1,t\right)\left(0,1\right)+∫_1^0F\left(t,\fracπ2t\right)\left(1,\fracπ2\right)$$'
    # math =r'\alpha'
    output = preprocess_for_transformer_old('I have a [imath]' + math.strip().strip('.').strip('$') + '[/imath] code to write a v c',
        )
    print("raw formula:", math)
    print(output)
    # print("processed formula:", count_textual_content_pya0(output))
