...
filepath = 'data_processing/processed/formula_blocks_pya0_formula_v2.txt'
outfile = 'data_processing/processed/train_set_formula_v2.txt'
...
def generate_pairs(cluster:list, target_len:int):
    ...
    for line in cluster:
        if line[0] == '[Q]':
            question_formulas.append(line[1])
        elif line[0] == '[R]':
            answer_formulas.append(line[1])
    output = []
...
def generate_small_data(cluster:list):
    target_len = 2
    longest_q_len = 0
    answer_formulas = []
    for line in cluster:
        if line[0] == '[Q]':
            if len(line[1])>longest_q_len:
                longest_q_len = len(line[1])
                query = line[1]
        elif line[0] == '[R]':
            answer_formulas.append(line[1])
    output = []
    if len(answer_formulas) >= 2:
        answer_formulas = random.sample(answer_formulas, k=target_len)
    for a in answer_formulas:
        output.append((query,a))
    return output
        ...
        for idx in ids:
            ...
            while True:
                ...
                if sample_tuple[0][0] == "[R]":
                    neg_samples.append(sample_tuple[0][1])
                    break
