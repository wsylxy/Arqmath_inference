...
def main() -> None:
    ...
    with torch.no_grad():
        ...
        for qid, query in topics:
            ...
            # print(qcode.dtype)
        batch_sz=401
        emb_files = sorted(glob.glob(os.path.join(cfg.DATA.OFFICIAL_EMBS_FILE, "word_emb.*.pt")), key=extract_number)
        doc_ids_files = sorted(glob.glob(os.path.join(cfg.DATA.OFFICIAL_EMBS_FILE, "doc_ids.*.pkl")), key=extract_number)
        doc_len_files = sorted(glob.glob(os.path.join(cfg.DATA.OFFICIAL_EMBS_FILE, "doc_len.*.pkl")), key=extract_number)
        # print(emb_files)
        # print(doc_ids_files)
        # print(doc_len_files)
        assert len(emb_files) == len(doc_ids_files) == len(doc_len_files)
        post_cnt = 0
        for path in doc_ids_files:
        ...
        for we_file, id_file, len_file in tqdm(zip(emb_files, doc_ids_files, doc_len_files), total=len(emb_files), desc="Loading embeddings"):
            print(1)
            flat_embs_all = torch.load(we_file, map_location='cpu')
